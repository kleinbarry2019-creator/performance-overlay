...
Für einen fertigen x64-Build genügt `PerformanceOverlay.exe`. Während ein Spiel aktiv ist, wird der FPS-Wert aus den tatsächlich präsentierten Frames über PresentMon berechnet. Auf dem Windows-Desktop, der Startoberfläche und normalen Fenstern verwendet das Overlay als Fallback die aktuelle DWM-Kompositions-/Bildschirmrate, damit dort ebenfalls durchgehend ein Live-Wert sichtbar bleibt. Wenn ein geschütztes Spiel wie Call of Duty die externe Frame-Telemetrie blockiert, bleibt der Spielwert für den konfigurierten geschützten Titel korrekt `--` statt eine erfundene Monitorrate anzuzeigen. GPU-Werte verwenden `nvidia-smi.exe`; die CPU-Temperatur wird absichtlich nicht in der Zeile angezeigt.
